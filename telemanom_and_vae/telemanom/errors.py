...
class Errors:
    def __init__(self, channel, config, run_id):
        ...
        self.n_windows_during_train = int((channel.y_train.shape[0] -
                              (self.config.batch_size * self.window_size))
                             / self.config.batch_size)
        ...
        # raw prediction error for train phase
        self.e_train = [abs(y_h - y_t[0]) for y_h, y_t in
                  zip(channel.y_hat_train, channel.y_train)]
        # raw prediction error for test phase
        ...
        if not len(channel.y_hat_train) == len(channel.y_train):
            raise ValueError('len(y_hat_train) != len(y_train): {}, {}'
                             .format(len(channel.y_hat_train), len(channel.y_train)))

        # smoothed prediction error
        self.e_s = pd.DataFrame(self.e).ewm(span=smoothing_window)\
            .mean().values.flatten()

        self.e_s_train = pd.DataFrame(self.e_train).ewm(span=smoothing_window) \
            .mean().values.flatten()
        ...
        if not channel.id == 'C-2':  # anomaly occurs early in window
            self.e_s_train[:self.config.l_s] = \
                [np.mean(self.e_s_train[:self.config.l_s * 2])] * self.config.l_s

        np.save(os.path.join('data', run_id, 'smoothed_errors', '{}.npy'
                             .format(channel.id)),
                np.array(self.e_s))

        np.save(os.path.join('data', run_id, 'smoothed_errors_train', '{}.npy'
                             .format(channel.id)),
                np.array(self.e_s_train))

        self.normalized = np.mean(self.e / np.ptp(channel.y_test))
        self.normalized_train = np.mean(self.e_s_train / (np.ptp(channel.y_train) if np.ptp(channel.y_train) != 0 else 1))
        logger.info("normalized prediction error during train: {0:.2f}"
                    .format(self.normalized_train))
